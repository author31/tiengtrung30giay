<template>
  <div class="header bg-blue-400 w-full e-h rounded-b-extra" :class="{onScroll : isScrolled}" >
    ...
    <div :class="{fadeOnScroll: isScrolled, enter: !isScrolled}" class="mx-4 -my-4 ">
      ...
      <button class="px-2 py-1 rounded-extra text-black submit" @click = "sendEmail">Xác nhận</button>
      
    </div>
    <div :class="{fadeOnScroll: isScrolled, enter: !isScrolled}" class="flex justify-end mx-4 -my-2">
        <img class="transform scale-x-180 -translate-y-20  sm:w-32  sm:-mx-2  sm:my-16" src="~/assets/logo/reading-side.svg" alt="" width="320" height="300">
    </div>
  </div>
</template>
